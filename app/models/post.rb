class Post < ApplicationRecord
  ...
  def validate_content_attachments_count
    if content.body.attachables.grep(ActiveStorage::Blob).count >   MAX_CONTENT_ATTCHMENT_COUNT
    errors.add(
      :content,
      :attachments_count_is_too_big,
      max_content_attachments_count: MAX_CONTENT_ATTCHMENT_COUNT
    )
    end
  end
